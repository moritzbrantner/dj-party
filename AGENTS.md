...
- The initial JavaScript-to-TypeScript conversion is behavior-preserving and uses TypeScript's `noCheck` emit while the existing dynamic DOM/WebAudio adapters are still being given explicit contracts. Do not hide that debt with broad `any` declarations merely to make a semantic checker quiet; ratchet semantic typing at stable authority and protocol boundaries in bounded follow-up slices.
...
4. `npm run typecheck` verifies that the TypeScript source graph parses and resolves under the pinned compiler without relying on generated JavaScript as source; semantic checking is intentionally ratcheted at explicit boundaries rather than faked with blanket `any` declarations during the mechanical migration.
