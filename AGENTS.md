...
- Multiplayer is not part of the local mixer/library slices. When introduced, keep signaling/session setup behind a narrow adapter to `multiplayer-setup-service`.
...
4. browser-module syntax validation passes for `web/app.js`, `web/mixer-app.js`, `web/library.js`, `web/library-imports.js`, `web/archive-import.js`, `web/analysis-cache.js`, `web/analysis-worker.js`, `web/performance.js`, `web/output-routing.js`, and `web/effects.js`
5. `node --test web/library.test.mjs web/archive-import.test.mjs web/analysis-cache.test.mjs` passes deterministic library/archive/cache helper contracts
6. `bash scripts/build-pages.sh`
7. the built Pages artifact contains the generated Rust/WASM package and all local browser modules/assets, including track-library, archive/playlist import, analysis-cache, deck-effects, and headphone-monitor routing assets
...
11. BPM Sync remains tempo-only while Phase Sync explicitly owns the transport seek needed for phase alignment
12. per-deck tone controls use Rust-owned parameter plans, exact center filter state is a bypass, and both master and pre-fader cue consume the same tone-shaped signal
13. browsers without usable audio-output selection keep monitoring disabled without weakening or redirecting normal master playback
