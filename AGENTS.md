...
4. browser-module syntax validation passes for `web/app.js`, `web/mixer-app.js`, `web/library.js`, `web/analysis-worker.js`, `web/performance.js`, and `web/output-routing.js`
5. `node --test web/library.test.mjs` passes the deterministic library helper contract
6. `bash scripts/build-pages.sh`
7. the built Pages artifact contains the generated Rust/WASM package and all local browser modules/assets, including track-library and headphone-monitor routing assets
...
10. browsers without usable audio-output selection keep monitoring disabled without weakening or redirecting normal master playback
